...
void MySet<T>::erase(Node<T> *ptr) {
	...
	if(!ptr->left && !ptr->right) {
		...
		if(ptr->parent) {
			if(ptr->parent->left == ptr) ptr->parent->left = child;
		...
		Node<T> *menor = ptr->right;
		while(menor->left) menor = menor->left;
		ptr->elem = menor->elem;
		erase(menor);
	}
}
...
template  <class T>
int MySet<T>::somaPares(Node<T> *ptr) const {
	if(!ptr) return 0;
	return (ptr->elem%2==0) + somaPares(ptr->left) + somaPares(ptr->right);
}


template  <class T>
int MySet<T>::somaParesIterativo(Node<T> *ptr) {
	if(!ptr) return 0;
	int ct = 0;
	stack<Node<T> *> s;
	s.push(ptr);
